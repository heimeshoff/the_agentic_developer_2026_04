  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-3 items-end flex-wrap">
      <div className="flex flex-col gap-1.5 w-40">
        <Label>{t('txCategory')}</Label>
        <Select value={categoryId} onValueChange={setCategoryId}>
          <SelectTrigger><SelectValue placeholder={t('txCategorySelect')} /></SelectTrigger>
          <SelectContent>
      ...
      <div className="flex flex-col gap-1.5 flex-1 min-w-32">
        <Label htmlFor="tx-desc">{t('txDescription')}</Label>
        <Input id="tx-desc" placeholder={t('txDescPlaceholder')} value={description} onChange={e => setDescription(e.target.value)} />
      </div>
      <div className="flex flex-col gap-1.5 w-32">
        <Label htmlFor="tx-amount">{t('txAmount')} ({currencySymbol})</Label>
